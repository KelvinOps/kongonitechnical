// app/courses/short-courses/page.tsx
'use client';
...
export default function ShortCoursesPage() {
  ...
  // Fetch short courses only
  const {
    data: shortCourses,
    isLoading,
    error,
  } = useQuery<CoursesResponse>({
    queryKey: ["/api/courses", { type: "short_course" }],
    queryFn: async (): Promise<CoursesResponse> => {
      const response = await fetch("/api/courses?type=short_course");
  ...
  const filteredCourses = useMemo((): Course[] => {
    if (!shortCourses) return [];
    
    if (searchTerm) {
      return shortCourses.filter((course: Course) =>
  ...
  const getIcon = (title: string): LucideIcon => {
    ...
    return Clock; // Default icon for short courses
  };

  const getIconColor = (index: number): string => {
    const colors = ["bg-purple-500", "bg-indigo-500", "bg-pink-500", "bg-blue-500", "bg-green-500", "bg-orange-500"];
    return colors[index % colors.length];
  };

  const getCourseLevel = (course: Course): string => {
    return course.level || 'Basic Level';
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-3xl font-bold text-red-600 mb-4">Error Loading Short Courses</h1>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-6">
            Our short courses are designed for quick skill acquisition and professional development. These flexible programs range from a few weeks to a few months, perfect for working professionals and those seeking specific skills enhancement.
          </p>
          <div className="flex items-center justify-center gap-6 text-sm text-gray-600">
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4" />
              <span>2 weeks - 6 months</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        <div className="mb-6 text-center">
          <p className="text-gray-600">
            Showing <span className="font-semibold">{filteredCourses.length}</span> short course{filteredCourses.length !== 1 ? 's' : ''}
            {searchTerm && (
        ...
        {filteredCourses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.map((course: Course, index: number) => {
              ...
              return (
                <Card key={course.id} className="p-6 hover:shadow-xl transition-all duration-300 hover:scale-105 border-l-4 border-l-purple-500">
                  <div className="flex items-start space-x-4">
